...
::: {.idea}
**Two parts, with one main purpose**
...
<img src="../../assets/lecture-map.svg" alt="A short introduction prepares two equally important application blocks: addition and subtraction.">
<figcaption>The introduction prepares the tools. Most of our time goes to using them.</figcaption>
</figure>
```
...
```{=html}
<nav class="lesson-route" aria-label="Introduction, two applications, and a final check">
<a href="#what-makes-a-vector"><span>1</span><strong>Introduction to vectors</strong><small>What the arrows mean, and how components describe them.</small></a>
<a href="#what-addition-means"><span>2A</span><strong>Addition: combine motions</strong><small>Draw, calculate with components, then practise.</small></a>
<a href="#subtracting-vectors"><span>2B</span><strong>Subtraction: find changes</strong><small>Draw, calculate with components, then practise.</small></a>
<a href="#check-your-reasoning"><span>✓</span><strong>Return to our goals</strong><small>See what you can now explain and calculate.</small></a>
</nav>
```

## Part 1 · Introduction to vectors {#what-makes-a-vector}
...
### A final check you can explain in words

- A swimmer moves across a flowing river. **Add or subtract?** Add the swimmer's
  velocity relative to the water to the current's velocity relative to the bank.
- A car's direction changes. **Add or subtract?** Subtract the before velocity
  from the after velocity.
- The calculation feels difficult. **Where can you start?** Draw the arrows,
  label what each means, and work with one component at a time.

::: {.recap}
**You have reached the goal: vector arithmetic for kinematics.**

You can combine motions and find changes using the parallelogram rule, the
distributive rule, and ordinary arithmetic with components. These are the vector
tools our kinematics work needs. The motion equations tell us how the objects behave.

If one step is still unclear, that gives us a useful question to discuss.
Try explaining your drawing to someone else, or bring it to class.
